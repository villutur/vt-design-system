  ...
  "inline-flex items-center rounded-full px-sm py-xs text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-primary/30 focus:ring-offset-2 focus:ring-offset-surface",
  {
    variants: {
      variant: {
        ...
        secondary: "border-transparent bg-surface-muted text-foreground shadow-sm hover:bg-surface-subtle",
        destructive: "border-transparent bg-error text-white shadow-sm hover:bg-error/80",
        success: "border-transparent bg-success text-white shadow-sm hover:bg-success/80",
        warning: "border-transparent bg-warning text-white shadow-sm hover:bg-warning/80",
        info: "border-transparent bg-primary text-white shadow-sm hover:bg-primary/80",
  ...
  ({ className, variant, size, dot, children, ...props }, ref) => {
    return (
      <div ref={ref} className={cn(badgeVariants({ variant, size }), className)} {...props}>
        {dot && <span className="mr-xs inline-block h-1.5 w-1.5 rounded-full bg-current" />}
        {children}
      </div>
    );
  },
